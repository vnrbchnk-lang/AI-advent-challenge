...
Учебный марафон по LLM и AI-агентам (курс mobiledeveloper.tech).
Ежедневные задания: минимальный Python-код, дёргает облачную LLM по REST.
...
- **Ключ:** переменная окружения `PROXYAPI_KEY` (в код/репо не коммитится).
...
> Ключ ProxyAPI с балансом нужен — запросы платные.
...
Если каталог скриптов pip не на PATH (`agent11` не находится) — запуск модулем:

```powershell
python -m assistant.cli11     # из папки week3
```

Память агентов (история диалога, слои памяти) лежит локально в JSON рядом с кодом, в
репо не коммитится (см. `.gitignore`); свежий клон стартует с пустой памятью.

Подробный контекст и журнал по дням — в [`CLAUDE.md`](CLAUDE.md) и
